...
- (void)startWithText:(NSString*)text delaySecs:(int)d config:(KFTypingConfig)cfg {
    ...
    __weak KFEngine *weakSelf = self;
    _engine.onProgress = [weakSelf](int done, int total) {
        __strong KFEngine *strongSelf = weakSelf;
        if (!strongSelf) return;
        dispatch_async(dispatch_get_main_queue(), ^{
            if (strongSelf.onProgress) strongSelf.onProgress(done, total);
        });
    };
    _engine.onDone = [weakSelf]() {
        ...
        if (!strongSelf) return;
        dispatch_async(dispatch_get_main_queue(), ^{
            if (strongSelf.onDone) strongSelf.onDone();
        });
    };
